...
if (!function_exists("setNewPassword")) {
	...
	function setNewPassword( $entityid,  $mode = 'view' ) {
		...
		$sql = sprintf("SELECT * FROM `entities` WHERE `entity-id` LIKE '%s'",$entityid);
		...
		foreach($emails as $entityid => $values)
		{
			foreach($values as $emailid => $email)
			{
				...
				if ($mailer->sendMail($to, array(),  array(), "New $mode password requested please utilise this email to recieve one!", $html, array(), NULL, true))
				{
					...
					$sql = "UPDATE `emails` SET `verified` = -100000 WHERE `email-id` LIKE '" . $emailid . "'";
...
if (!function_exists("getPeerIdentity")) {
	...
	function getPeerIdentity( $url,  $short, $version, $callback, $polinating = true, $root = "http://entities.labs.coop" ) {
		...
		$sql = "SELECT * FROM `peers` WHERE `api-url` LIKE '%s'";
		if ($GLOBALS['EntitiesDB']->getRowsNum($results = $GLOBALS['EntitiesDB']->queryF(sprintf($sql, mysql_escape_string($url))))==1)
		{
			...
			if (strpos($url, 'localhost')>0)
				$polinating = false;
			$sql = "INSERT INTO `peers` (`peer-id`, `api-url`, `api-short-url`, `version`, `callback`, `created`) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')";
			$peerid = md5($url.$version.$callback.$polinating.$root.microtime(true));
			if ($GLOBALS['EntitiesDB']->queryF(sprintf($sql, mysql_escape_string($peerid), mysql_escape_string($url), mysql_escape_string($shorturl), mysql_escape_string($version), mysql_escape_string($callback), time())))
			{
	...
	function getEntityArray($entity = array())
	{
		...
		foreach($entity as $field => $value)
		{
			if (!empty($value))
				...
				{
					$sql = sprintf("SELECT * FROM `fingerprints` WHERE `entity-id` LIKE '%s' AND `fingerprint` LIKE '%s'", $entity['entity-id'], $value);
					if ($GLOBALS['EntitiesDB']->getRowsNum($results = $GLOBALS['EntitiesDB']->queryF($sql))==1)
					{
						$finger = $GLOBALS['EntitiesDB']->fetchArray($results);
	...
	function getFingeredArray($table = 'unknown', $hash = '')
	{
		...
		if (!empty($key))
		{
			switch ($table)
			{
				...
				case "entities":
				case "imports":
					if ($GLOBALS['EntitiesDB']->getRowsNum($results = $GLOBALS['EntitiesDB']->queryF(sprintf($sql, $table, $key, $hash)))>0)
					{
						$ret = array('data' => $GLOBALS['EntitiesDB']->fetchArray($results));
						$ret['record']['ident'] = $ret['data'][$key];
						$ret['record']['prime'] = $key;
						$ret['record']['table'] = $table;
						unset($ret['data'][$key]);
						$ret['record']['fields'] = array_keys($ret['data']);
						if ($GLOBALS['EntitiesDB']->getRowsNum($results = $GLOBALS['EntitiesDB']->queryF(sprintf($sql, 'fingerprints', 'fingerprint', $hash)))>0)
						{
							while($finger = $GLOBALS['EntitiesDB']->fetchArray($results))
							{
...
if (!function_exists("getPlaceID")) {
	...
	function getPlaceID($countryid = '', $name = '', $countriessupported = array())
	{
		...
		if (!empty($count))
			$place = json_decode(getURIData('http://places.labs.coop/v1/$count/$name/json.api', 120, 120), true);
		else
			foreach($countries['countries'] as $country => $values)
			{
				$country = strtolower(str_replace(" ", "", $country));
				$place = json_decode(getURIData('http://places.labs.coop/v1/$country/$name/json.api', 120, 120), true);
		...
		foreach( $data as $entityid => $values)
		{
			...
			if (!empty($values['addresses']))
			{
				foreach($values['addresses'] as $type => $address)
				{
					...
					$sql = "SELECT `address-id` FROM `addresses` WHERE ";
					foreach($address as $field => $value)
						$sql .= " `$field` = '" . mysql_escape_string($value) . "' AND";
					$sql = substr($sql, 0, strlen($sql)-4);
					if ($GLOBALS['EntitiesDB']->getRowsNum($results = $GLOBALS['EntitiesDB']->queryF($sql))==0)
					{
						...
						$sql = "INSERT INTO `addresses` (`" . implode("`, `", array_keys($address)) . "`) VALUES ('" . implode("', '", $address) . "')";
						if (!$GLOBALS['EntitiesDB']->queryF($sql))
							die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
						
					} else
						if (list($addressid) = $GLOBALS['EntitiesDB']->fetchRow($results))
						{
							$address['address-id'] = $addressid;
							$sql = "UPDATE `addresses` SET `entities` = `entities` + 1 WHERE `address-id` = '$addressid'";
							if (!$GLOBALS['EntitiesDB']->queryF($sql))
								die("Update Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
						}
					...
					$sql = "INSERT INTO `fingerprints` (`entity-id`, `import-id`, `peer-id`, `type`, `fingerprint`, `created`) VALUES ('$entityid', '$importid', '$peerid',  'addresses','$addressid', '".time()."')";
					if (!$GLOBALS['EntitiesDB']->queryF($sql))
						die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
					switch($type)
					{
			...
			if (!empty($values['phones']))
			{
				foreach($values['phones'] as $type => $number)
				{
					...
					$sql = "SELECT `phone-id` FROM `phones` WHERE ";
					...
					if ($GLOBALS['EntitiesDB']->getRowsNum($results = $GLOBALS['EntitiesDB']->queryF($sql))==0)
					{
						...
						$sql = "INSERT INTO `phones` (`" . implode("`, `", array_keys($phone)) . "`) VALUES ('" . implode("', '", $phone) . "')";
						if (!$GLOBALS['EntitiesDB']->queryF($sql))
							die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
					} else
						if (list($phoneid) = $GLOBALS['EntitiesDB']->fetchRow($results))
						{
							$phone['phone-id'] = $phoneid;
							$sql = "UPDATE `phones` SET `entities` = `entities` + 1 WHERE `phone-id` = '$phoneid'";
							if (!$GLOBALS['EntitiesDB']->queryF($sql))
								die("Update Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
						}
					$sql = "SELECT * FROM `peers` WHERE `peer-id` NOT LIKE '%s' AND `polinating` = 'Yes'";
					...
					$sql = "INSERT INTO `fingerprints` (`entity-id`, `import-id`, `peer-id`, `type`, `fingerprint`, `created`) VALUES ('$entityid', '$importid', '$peerid',  'phones','$phoneid', '".time()."')";
					if (!$GLOBALS['EntitiesDB']->queryF($sql))
						die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
					if (isset($phoneid))
			...
			if (!empty($values['email']))
			{
				foreach($values['email'] as $type => $dvalues)
				{
					...
					$sql = "SELECT `email-id` FROM `emails` WHERE ";
					...
					if ($GLOBALS['EntitiesDB']->getRowsNum($results = $GLOBALS['EntitiesDB']->queryF($sql))==0)
					{
						...
						$sql = "INSERT INTO `emails` (`" . implode("`, `", array_keys($email)) . "`) VALUES ('" . implode("', '", $email) . "')";
						if (!$GLOBALS['EntitiesDB']->queryF($sql))
							die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
					} else
						if (list($emailid) = $GLOBALS['EntitiesDB']->fetchRow($results))
						{
							$email['email-id'] = $emailid;
							$email['type'] = "Email";
							$sql = "UPDATE `emails` SET `entities` = `entities` + 1 WHERE `email-id` = '$emailid'";
							if (!$GLOBALS['EntitiesDB']->queryF($sql))
								die("Update Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
						}
					$sql = "SELECT * FROM `peers` WHERE `peer-id` NOT LIKE '%s' AND `polinating` = 'Yes'";
					...
					$sql = "INSERT INTO `fingerprints` (`entity-id`, `import-id`, `peer-id`, `type`, `fingerprint`, `created`) VALUES ('$entityid', '$importid', '$peerid',  'emails','$emailid', '".time()."')";
					if (!$GLOBALS['EntitiesDB']->queryF($sql))
						die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
					if (isset($emailid))
			...
			if (!empty($values['strings']))
			{
				foreach($values['strings'] as $type => $dvalues)
				{
					$string = array();
					$string['data'] = $dvalues; 
					$sql = "SELECT `string-id` FROM `strings` WHERE ";
					foreach($email as $field => $value)
						$sql .= " `$field` = '" . mysql_escape_string($value) . "' AND";
					$sql = substr($sql, 0, strlen($sql)-4);
					if ($GLOBALS['EntitiesDB']->getRowsNum($results = $GLOBALS['EntitiesDB']->queryF($sql))==0)
					{
						...
						$sql = "INSERT INTO `strings` (`" . implode("`, `", array_keys($string)) . "`) VALUES ('" . implode("', '", $string) . "')";
						if (!$GLOBALS['EntitiesDB']->queryF($sql))
							die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
					} else
						if (list($stringid) = $GLOBALS['EntitiesDB']->fetchRow($results))
						{
					...
					$sql = "INSERT INTO `fingerprints` (`entity-id`, `import-id`, `peer-id`, `type`, `fingerprint`, `created`) VALUES ('$entityid', '$importid', '$peerid',  'strings','$stringid', '".time()."')";
					if (!$GLOBALS['EntitiesDB']->queryF($sql))
						die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
			...
			if (!empty($values['keywords']))
			{
				foreach($values['keywords'] as $id => $keyword)
				{
					...
					{
						$sql = "SELECT `keyword-id` FROM `keywords` WHERE `keyword` LIKE '" . mysql_escape_string($keyword) . "'";
						if ($GLOBALS['EntitiesDB']->getRowsNum($results = $GLOBALS['EntitiesDB']->queryF($sql))==0)
						{
							...
							if (!empty($key))
							{
								...
								$sql = "INSERT INTO `keywords` (`" . implode("`, `", array_keys($key)) . "`) VALUES ('" . implode("', '", $key) . "')";
								if (!$GLOBALS['EntitiesDB']->queryF($sql))
									die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
							}
						} else
							if (list($keywordid) = $GLOBALS['EntitiesDB']->fetchRow($results))
							{
								$key['keyword-id'] = $keywordid;
								$sql = "UPDATE `keywords` SET `entities` = `entities` + 1 WHERE `keyword-id` = '$keywordid'";
								if (!$GLOBALS['EntitiesDB']->queryF($sql))
									die("Update Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
							}
						$sql = "SELECT * FROM `peers` WHERE `peer-id` NOT LIKE '%s' AND `polinating` = 'Yes'";
						...
						$sql = "INSERT INTO `keywords_entities` (`entity-id`, `keyword-id`) VALUES ('$entityid', '$keywordid')";
						if (!$GLOBALS['EntitiesDB']->queryF($sql))
							die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
								
						$sql = "INSERT INTO `fingerprints` (`entity-id`, `import-id`, `peer-id`, `type`, `fingerprint`, `created`) VALUES ('$entityid', '$importid', '$peerid',  'keywords','$keywordid', '".time()."')";
						if (!$GLOBALS['EntitiesDB']->queryF($sql))
							die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
					}
				}
			...
			if (!empty($values['categories']))
			{
				...
				{
					$sql = "INSERT INTO `categories_entities` (`entity-id`, `category-id`) VALUES ('$entityid', '$categoryid')";
					if (!$GLOBALS['EntitiesDB']->queryF($sql))
						die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
					$sql = "UPDATE `categories` SET `entities` = `entities` + 1 WHERE `category-id` = '$categoryid'";
					if (!$GLOBALS['EntitiesDB']->queryF($sql))
						die("Update Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
			...
			$sql = "INSERT INTO `entities` (`" . implode("`, `", array_keys($entity)) . "`) VALUES ('" . implode("', '", $entity) . "')";
				if (!$GLOBALS['EntitiesDB']->queryF($sql))
					die("Insert SQL Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
			$sql = "SELECT * FROM `peers` WHERE `peer-id` NOT LIKE '%s' AND `polinating` = 'Yes'";
			if ($GLOBALS['EntitiesDB']->getRowsNum($results = $GLOBALS['EntitiesDB']->queryF(sprintf($sql, mysql_escape_string($GLOBALS['peerid']))))>=1)
			...
			$sql = "UPDATE `imports` SET `records` = `records` + 1 WHERE `import-id` = '$importid'";
			if (!$GLOBALS['EntitiesDB']->queryF($sql))
				die("Update Failed: $sql :: " . $GLOBALS['EntitiesDB']->error());
		}
		return $ret;
	...
	function getHTMLForm($mode = '', $clause = '', $callback = '', $output = '', $version = 'v2')
	{
		...
		switch ($mode)
		{
			case "uploads":
				...
				$countries = json_decode(getURIData('http://places.labs.coop/v1/list/list/json.api', 120, 120), true);
				$i=0;
				foreach($countries['countries'] as $name => $country)
				{
					$i++;
					$form[] = "\t\t\t\t\t\t<td>";
					$form[] = "\t\t\t\t\t\t\t<input type='checkbox' name='country[".$country['key']."]' id='country' value='".$country['key']."' />&nbsp;".$name.(!empty($country['TLD'])?" (".$country['TLD'].')':"")."&nbsp;&nbsp;";
					$form[] = "\t\t\t\t\t\t</td>";
					if ($i==6)
					{
						$form[] = "\t\t\t\t\t<tr>";
...
if (!function_exists("getIPIdentity")) {
	...
	function getIPIdentity($ip = '', $sarray = false)
	{
		...
		if (!$row = $GLOBALS['EntitiesDB']->fetchArray($GLOBALS['EntitiesDB']->queryF($sql['selecta'])))
			if (($ipaddypart[0] ===  $serverpart[0] && $ipaddypart[1] ===  $serverpart[1]) )
			{
				...
				if (!$row = $GLOBALS['EntitiesDB']->fetchArray($GLOBALS['EntitiesDB']->queryF($sql['selectb'])))
				{
					$row = array();
					$row['ipaddy'] = $data['ip'];
					if (!filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) === false)
					...
					foreach($whoisuris as $uri)
					{
						...
						{
							$row['whois'][$row['type']] = json_decode(getURIData(sprintf($uri, $row['ipaddy'], 'json'), 120, 120), true);
						} elseif (empty($row['whois']['domain']) || !isset($row['whois']['domain']))
						{
							$row['whois']['domain'] = json_decode(getURIData(sprintf($uri, $row['domain'], 'json'), 120, 120), true);
						} else
							continue;
					}
					$row['ip-id'] = md5(json_encode($row));
					$data = array();
					...
					$GLOBALS['EntitiesDB']->queryF($sql['inserta']);
				} 
			} 
		$sql['updatea'] = "UPDATE `networking` SET `last` = '". time() . '\' WHERE `ip-id` = "' . $row['ip-id'] .'"';
		$GLOBALS['EntitiesDB']->queryF($sql['updatea']);
		if ($sarray == false)
			return $row['ip-id'];
		else
			return $row;
	...
	*/
	function getUploadHTML($return = '')
	{
		$forms = array();
			...
			$forms[md5($return)]['html'] = getURIData($GLOBALS['source']."v2/uploads/forms.api", 83, 83, array('return'=>$return));
...
if (!function_exists("getMimetype")) {
	...
	{
		$result = $GLOBALS['EntitiesDB']->queryF("SELECT * from `mimetypes` WHERE `type` LIKE '$type'");
		while($row = $GLOBALS['EntitiesDB']->fetchArray($result))
			return $row['mimetype'];
		return 'text/html';
	}
...
if (!function_exists("getExampleNodes")) {
	...
	{
		$nodes = array();
		$result = $GLOBALS['EntitiesDB']->queryF("SELECT * from `categories` WHERE LENGTH(`category`) > 3 ORDER BY RAND() LIMIT " . mt_rand(1,3));
		while($row = $GLOBALS['EntitiesDB']->fetchArray($result))
			$nodes[] = str_replace(" ", "-", strtolower($row['category']));
		$result = $GLOBALS['EntitiesDB']->queryF("SELECT * from `keywords` WHERE LENGTH(`keyword`) > 3 ORDER BY RAND() LIMIT " . mt_rand(1,3));
		while($row = $GLOBALS['EntitiesDB']->fetchArray($result))
...
if (!function_exists("getExampleFingerprint")) {
	...
	{
		$nodes = array();
		$result = $GLOBALS['EntitiesDB']->queryF("SELECT * from `entities` WHERE `view-protected` = 'No' ORDER BY RAND() LIMIT 1");
		while($row = $GLOBALS['EntitiesDB']->fetchArray($result))
			return $row;
	}
}
...
if (!function_exists("getExampleCsvFiles")) {
	...
	{
		$entities = array();
		$result = $GLOBALS['EntitiesDB']->queryF("SELECT * from `entities_archiving` WHERE `entities_id` = '$md5'");
		while($row = $GLOBALS['EntitiesDB']->fetchArray($result))
			if (!empty($row['type']))
...
if (!function_exists("getCompleteCsvListAsArray")) {
	function getCompleteCsvListAsArray($dirname, $result = array())
	{
		foreach(getDirListAsArray($dirname) as $path)
		{
			foreach(getCsvListAsArray($dirname . DIRECTORY_SEPARATOR . $path) as $file)
...
if (!function_exists("getCsvCullList")) {
	function getCsvCullList($files = array())
	{
		...
		$sql = "SELECT COUNT(*) as RC from `entities_fingering` where `fingerprint` LIKE '%s'";
		foreach($ret as $fingerprint => $filevars)
		{
			list($count) = $GLOBALS['EntitiesDB']->fetchRow($GLOBALS['EntitiesDB']->queryF(sprintf($sql, $fingerprint)));
			if ($count<1)
 ...
 * @var string
 */
if (!isset($GLOBALS['peerid']))
	$GLOBALS['peerid'] = getPeerIdentity(API_URL, API_VERSION, API_URL_CALLBACK, API_POLINATING, API_ROOT_NODE);
