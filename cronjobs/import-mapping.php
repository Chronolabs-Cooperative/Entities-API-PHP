...
$maps = array();
$sql = "SELECT * FROM `imports_maps` WHERE `state` LIKE 'Defined'";
if (!$results = $GLOBALS['EntitiesDB']->queryF($sql))
	die('SQL Failed: : ' . $sql . " :: " . $GLOBALS['EntitiesDB']->error());
while ($map = $GLOBALS['EntitiesDB']->fetchArray($results))
	$maps[$map['maps-id']] = $map;
if (!count($maps))
	die('Recordset Failed: : ' . $sql . " :: " . $GLOBALS['EntitiesDB']->error());
			...
			foreach(explode(DIRECTORY_SEPARATOR, $file) as $key => $email)
			{
				if (checkEmail($email))
				{
					...
					if ($GLOBALS['EntitiesDB']->getRowsNum($result = $GLOBALS['EntitiesDB']->queryF($sql))>=1)
					{
						...
						$sql = "SELECT * FROM `imports` WHERE `maps-id` IN ('".implode("', '", array_keys($maps))."') AND  `uploader-email-id` LIKE '".$uploader['email-id']."' AND `filename` LIKE '".mysql_escape_string(basename($file))."' AND `path` LIKE '".mysql_escape_string(dirname($file))."'"; 
						if ($GLOBALS['EntitiesDB']->getRowsNum($result = $GLOBALS['EntitiesDB']->queryF($sql))==1)
						{
							$data = array();
							...
							$cats = array();
							$sql = "SELECT * FROM `categories_codes` WHERE `maps-id` LIKE '".$import['maps-id']."' AND `code` IN ('" . implode("', '", $codesql) . "') ORDER BY `code` ASC";
							if (!$results = $GLOBALS['EntitiesDB']->queryF($sql))
								die('SQL Failed: : ' . $sql . " :: " . $GLOBALS['EntitiesDB']->error());
							...
							if (count($cats) != count($codes))
							{
								if ($uploader['contacted'] < time() - (3600 * 24 * mt_rand(2.75, 4.5)))
								{
									...
									{
										$sql = "UPDATE `emails` SET `verified` = -100 WHERE `email-id` LIKE '" . $uploader['email-id'] . "'";
