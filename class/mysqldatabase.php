...
class DebauchMySQLDatabase extends DebauchDatabase
{
    ...
    public function connect($selectdb = true)
    {
        ...
        if (DB_ENTITIES_PERS == true) {
            $this->conn = @mysql_pconnect(DB_ENTITIES_HOST, DB_ENTITIES_USER, DB_ENTITIES_PASS);
        } else {
            $this->conn = @mysql_connect(DB_ENTITIES_HOST, DB_ENTITIES_USER, DB_ENTITIES_PASS);
        }
        ...
        if (!$this->conn) {
			...
			if ($selectdb != false) {
				if (!mysql_select_db(DB_ENTITIES_NAME)) {
					die('Could not select: ' . mysql_error());
				}
			}
        if (!isset($db_charset_set) && defined('DB_ENTITIES_CHAR') && DB_ENTITIES_CHAR) {
            $this->queryF("SET NAMES '" . DB_ENTITIES_CHAR . "'");
        }
        $db_charset_set = 1;
