 ...
 */
include_once (dirname(__FILE__).'/mail/entitiesmultimailer.php');

/**
 * Class for sending mail.
 *
 * Changed to use the facilities of  {@link EntitiesMultiMailer}
 *
 * @package class
 * @subpackage mail
 * @author Kazumi Ono <onokazu@xoops.org>
 */
class EntitiesMailer
{
    /**
     * reference to a {@link EntitiesMultiMailer}
     *
     * @var EntitiesMultiMailer
     * @access private
     ...
     * Constructor
     *
     * @return EntitiesMailer
     */
    function EntitiesMailer($from, $fromname, $method = 'SMTPAuth')
    {
        $this->multimailer = new EntitiesMultiMailer($from, $fromname, $method);
        $this->reset();
        $this->from = $from;
     ...
     * Send email
     *
     * Uses the new EntitiesMultiMailer
     *
     * @param string $
     * @param string $
     * @param string $
    ...
    function sendMail($email = array(), $cc = array(), $bcc = array(), $subject = '', $body = '', $attachments = array(), $headers, $ishtml = false)
    {
        ...
        foreach($email as $addy => $name)
        	if (is_string($addy) && is_string($name) && !is_numeric($addy))
        		$this->multimailer->AddAddress($addy, $name);
        	else
        		$this->multimailer->AddAddress($name, $name);
        foreach($cc as $addy => $name)
        	if (is_string($addy) && is_string($name) && !is_numeric($addy))
        		$this->multimailer->AddCC($addy, $name);
        	else 
        		$this->multimailer->AddCC($name, $name);
        foreach($bcc as $addy => $name)
        	if (is_string($addy) && is_string($name) && !is_numeric($addy))
        		$this->multimailer->AddBCC($addy, $name);
        	else 
        		$this->multimailer->AddBCC($name, $name);
